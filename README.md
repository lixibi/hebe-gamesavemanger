...
`cloudServerURL`、本机存档路径、游戏程序路径和启动参数都只保存在当前客户端。云端只保存游戏名、游戏 ID、云端文件夹名，方便另一台电脑连接云服务后自动下发游戏列表，再分别设置该电脑自己的本地路径。
...
- 左侧可设置云服务地址，支持直接填写 `IP:27843`，点击测试连接，保存后会重新下发云端游戏列表。
